...
func _show_subtitles(text: String):
	...
	text_tween.tween_property(subtitle_label, "visible_characters", char_count, clamp(char_count * 0.028, 0.8, 6.0))
...
func _on_transcription_metrics(text: String, _filler_count: int, _wpm: float):
	print("You said: " + text)
	llm.ask(text)
